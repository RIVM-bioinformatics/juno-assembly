...
output_dir = "/data/BioGrid/singhsp/Juno_assembly/output" #path given as -o in pipeline
...
def get_phred_score(phred: str) -> pd.DataFrame:
    ...
    phred = phred_data['report_plot_data']['fastqc_per_sequence_quality_scores_plot']['datasets']
    ...
    parsed_df = df.groupby('name').data.apply(lambda x: pd.DataFrame(x.values[0])).reset_index()

    parsed2_df = parsed_df.drop('level_1', axis = 1)
    parsed2_df.rename(columns = {0:'phred', 1: 'Sequences'}, inplace = True)
    parsed3_df = parsed2_df[parsed2_df['name'].str.contains("_pR") == False] # Remove reads containing _pR

    parsed4_df  = parsed3_df.groupby('name')['Sequences'].max().reset_index() # get phred score by taking the score associated with the highest number of sequences per read
    parsed5_df = parsed3_df.merge(parsed4_df)
    parsed5_df['name'] = parsed5_df['name'].apply(lambda x: x.split('_')[0]) #name to match metadata
...
def get_sequence_len(seq_len: str) -> pd.DataFrame:
    ...
    seq_len_df = pd.read_csv(seq_len_filepath, sep= '\t', usecols=['Sample', 'avg_sequence_length'])
    seq_len_df.rename(columns = {'Sample':'sample'}, inplace = True)
    seq_len_df = seq_len_df[seq_len_df['sample'].str.contains("_pR") == False] # remove reads containing _pR
    
    seq_len_df['sample'] = seq_len_df['sample'].apply(lambda x: x.split('_')[0]) # sample name to match metadata
